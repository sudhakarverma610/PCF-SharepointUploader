  ...
  implements ComponentFramework.StandardControl<IInputs, IOutputs>
{
  ...
  public init(
    ...
    if (entityRef) {
      ...
      this.uploadProps.controlToRefresh =
        context.parameters.ControlNameForRefresh.raw;
      this.uploadProps.uploadIcon = this.getImageBase64();
      this.uploadProps.useNoteAttachment =
        context.parameters.UseNoteAttachment.raw === "1";
      this.uploadProps.context = context;
    }
  ...
  private retrieveEntitySetNameAndRender(
    context: ComponentFramework.Context<IInputs>,
    entityName: string,
  ) {
    var thisRef = this;
    if (Config.Environment == Environment.Local) {
      thisRef.uploadProps.entitySetName = "contact";
      thisRef.renderComponent();
      return;
    }
    context.utils.getEntityMetadata(entityName).then(
      function (response) {
      ...
      function (errorResponse: any) {
        console.log(
          `Error occurred while retrieving the entity metadata. ${errorResponse}`,
        );
      },
    );
  }
  ...
  private renderComponent() {
    ReactDOM.render(
      React.createElement(AttachmentUploader, this.uploadProps),
